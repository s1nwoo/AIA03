...
from emp
where hiredate like '%04%' -- 다른 수식으로 바꿔야함...
order by ename;
...
select to_char(hiredate, 'YY/MON DY') as "입사 일(년,월,요일)"
from emp;


-- 20. 올해 몇 칠이 지났는지 출력하시오. 현재날짜에서 올해 1월 1일을 뺀 결과를 출력하고 TO_DATE 함수를 사용하여 데이터 형을 일치 시키시오.
select trunc(sysdate - to_date('2020/01/01', 'YYYY/DD/MM')) as "올해 지난 일"
from dual;
...
select ename, job, sal, DECODE(job, 'ANALYST', sal+200,
                                    'SALESMAN', sal+180,
                                    'MANAGER', sal+150,
                                    'CLERK', sal+100, sal) as "인상 후 총액"
from emp
order by "인상 후 총액";


-- 23. 모든 사원의 급여 최고액, 최저액, 총액 및 평균 급여를 출력하시오. 평균에 대해서는 정수로 반올림하시오.
select max(sal) as "급여 최고액", min(sal) as "급여 최저액", sum(sal) as "급여 총액", round(avg(sal),0) as "급여 평균"
from emp;


-- 24. 각 담당 업무 유형별로 급여 최고액, 최저액, 총액 및 평균 액을 출력하시오. 평균에 대해서는 정수로 반올림 하시오.
select job, max(sal) as "급여 최고액", min(sal) as "급여 최저액", sum(sal) as "급여 총액", round(avg(sal),0) as "급여 평균"
from emp
group by job
order by job;
...
-- 26. 관리자 수를 나열하시오.
select count(mgr) as "관리자 수"
from emp;


-- 27. 급여 최고액, 급여 최저액의 차액을 출력하시오.
select max(sal) - min(sal) as "최고 급여 - 최저 급여"
from emp;


-- 28. 직급별 사원의 최저 급여를 출력하시오. 관리자를 알 수 없는 사원의 최저 급여가 2000 미만인 그룹은 제외시키고 
--     결과를 급여에 대한 내림차순으로 정렬하여 출력하시오.
select job, min(sal) as "최저 급여"
from emp
group by job
having min(sal) > 2000
...
select deptno, decode(deptno, 10, 'ACCOUNTING', 
                ...
                decode(deptno, 10, 'NEW YORK', 
                               ...
                               30, 'CHICAGO', 
                               40, 'BOSTON') as "지역명",
count(*) as "사원수", round(avg(sal)) as "평균 급여"
from emp 
group by deptno
order by deptno;
...
select job, deptno, decode(deptno, 10, sum(sal)) as "부서 10",
