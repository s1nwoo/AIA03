...
public class PhoneInfoHandler {
	...
	private static PhoneInfoHandler hand = new PhoneInfoHandler(100);
	...
	private PhoneInfoHandler(int num) {
		myPhone = new ArrayList<PhoneInfo>();
	}
...
//		}
		...
		while (true) {
			...
			case MenuInterface.GAME:
				...
				info = new PhoneGameInfo(name, phoneNumber, address, email, cafeName, nickName);
				break;
			}

		
		// return을 통해 main으로 돌아간다.
		addPhoneInfo(info);
		return;
	}
	...
	int searchIndex(String name) {
		...
		for (int i = 0; i < myPhone.size(); i++) {
			if (myPhone.get(i).name.equals(name)) {
				searchIndex = i;
				break;
			}
		}
	...
	void showInfo() {
		...
		if (index < 0) {
			...
			System.out.println("---------------");
			myPhone.get(index).showBasicInfo();
			System.out.println("---------------");
		}
	...
	void editInfo () {
		...
		if (index < 0) {
			...
			String editName = myPhone.get(index).name;
			...
			if (myPhone.get(index) instanceof PhoneUnivInfo) {
			...
			} else if (myPhone.get(index) instanceof PhoneCompanyInfo) {
			...
			} else if (myPhone.get(index) instanceof PhoneGameInfo) {
				System.out.println("동호회 이름을 입력하세요");
				String cafeName = kb.nextLine();
				System.out.println("닉네임을 입력하세요");
				String nickName = kb.nextLine();

				info = new PhoneGameInfo(editName, phoneNumber, address, email, cafeName, nickName);

			}

			// 배열에 새로운 인스턴스를 저장
			myPhone.add(index, info);
