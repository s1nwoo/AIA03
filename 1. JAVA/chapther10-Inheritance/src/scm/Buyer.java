...
public class Buyer {
	...
	void buy(Product p) {
		...
		if(money < p.price) {
			System.out.println(+p.price-+money+"원이 부족하여 "+p+"를 구매할 수 없습니다.");
			return;
		}
		
		// 구매 처리
		this.money -= p.price; // 보유한 금액에서 제품의 금액을 차감
		this.bonusPoint += p.bonusPoint; // 포인트 추가
...
//		numOfProduct++;
	...
	void summary() {
		...
		int sum = 0; // 구매한 제품의 총액
		int points = 0; // 구매한 제품들의 포인트 총합
		
		String itemList = ""; // 구매한 제품의 목록
		...
		System.out.println("구매하신 제품목록은 "+itemList+"입니다");
		System.out.println("구매하신 제품들의 총 금액은 "+sum+"원 입니다");
		System.out.println("구매하신 제품들의 총 포인트는 "+points+"포인트 입니다");
	}
